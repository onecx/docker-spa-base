FROM nginxinc/nginx-unprivileged:1.29.8-alpine@sha256:f35982400455b4f359083ba9d1ef573fbbe4221a6a8a194dc7a14beb4e2cd3dc

ENV DIR_NGINX /etc/nginx
ENV DIR_SERVER_BLOCKS ${DIR_NGINX}/conf.d
ENV DIR_LOCATION ${DIR_SERVER_BLOCKS}/locations
ENV DIR_HTML /usr/share/nginx/html
ENV DIR_STATIC ${DIR_HTML}/static
ENV DIR_ASSETS ${DIR_HTML}/assets

ENV CORS_ENABLED false
ENV CORS_ALLOW_ORIGIN ""
ENV CORS_ALLOW_ORIGIN_REGEX "^$"
ENV CORS_ALLOW_CREDENTIALS false
ENV CORS_ALLOW_HEADERS "Authorization, Origin, X-Requested-With, Content-Type, Accept"
ENV CORS_ALLOW_METHODS "GET, POST, OPTIONS, HEAD"

USER root
RUN apk update && \
    apk add jq && \
    rm -rf ${DIR_HTML}/* && \
    mkdir -p ${DIR_LOCATION} && \
    # chown 1001 -R ${DIR_SERVER_BLOCKS} && \
    chmod 775 -R ${DIR_SERVER_BLOCKS} && \
    chmod 775 -R ${DIR_LOCATION}

# setup entry point
COPY entrypoint.sh /
RUN chmod +x /entrypoint.sh

# nginx default configuration
COPY 00-cors-map.conf ${DIR_SERVER_BLOCKS}/
COPY locations/*.conf ${DIR_LOCATION}/
COPY default.conf ${DIR_SERVER_BLOCKS}

# default error pages
COPY error_pages/*.html ${DIR_STATIC}/

# default list of environment variable names
ENV CONFIG_ENV_LIST BFF_URL,APP_BASE_HREF,CORS_ENABLED,CORS_ALLOW_ORIGIN,CORS_ALLOW_ORIGIN_REGEX,CORS_ALLOW_CREDENTIALS,CORS_ALLOW_HEADERS,CORS_ALLOW_METHODS,APP_VERSION,APP_ID,PRODUCT_NAME,TKIT_PORTAL_URL
# RUN chown -R 1001:1001 /var && mkdir -p /var/run && touch /var/run/nginx.pid && chmod 775 -R /var/run/nginx.pid

ENTRYPOINT ["/bin/sh", "/entrypoint.sh"]
CMD ["nginx", "-g", "daemon off;"]

# Default build user root, runtime user 1001
# USER 1001
